...
@section('page-title','Edit Test Panel')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.test-panels.update', $testPanel) }}" class="space-y-5">
            @csrf @method('PUT')
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Panel Name *</label>
                    <input type="text" name="name" value="{{ old('name', $testPanel->name) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                </div>
                <div>
                    ...
                    <select name="category_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                        ...
                        <option value="{{ $c->id }}" {{ old('category_id',$testPanel->category_id)==$c->id?'selected':'' }}>{{ $c->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div>
                ...
                <div class="border border-gray-200 rounded-xl p-3 max-h-64 overflow-y-auto space-y-1">
                    ...
                    <label class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-gray-50 cursor-pointer text-sm text-gray-700">
                        <input type="checkbox" name="tests[]" value="{{ $test->id }}" {{ in_array((string)$test->id, array_map('strval', old('tests', $selectedTests ?? [])))?'checked':'' }} class="rounded text-indigo-600 focus:ring-indigo-500">
                        {{ $test->name }}
                    </label>
                    @endforeach
                    @endforeach
