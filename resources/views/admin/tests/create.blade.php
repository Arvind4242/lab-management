        ...
        <form method="POST" action="{{ route('admin.tests.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="default_result" value="{{ old('default_result') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="e.g. 70–110 mg/dL">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Ref. Range (Female)</label>
                    <input type="text" name="default_result_female" value="{{ old('default_result_female') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Ref. Range (Other)</label>
                    <input type="text" name="default_result_other" value="{{ old('default_result_other') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Test Group</label>
                    <input type="text" name="test_group" value="{{ old('test_group') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="e.g. Biochemistry">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Price (₹)</label>
                    <input type="number" name="price" value="{{ old('price') }}" step="0.01" min="0" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                </div>
            </div>
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-colors">Create Test</button>
