...
@section('content')
<form method="POST" action="{{ route('user.reports.store') }}" id="reportForm">
@csrf
    ...
    <div class="lg:col-span-1 space-y-5">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="date" name="test_date" value="{{ old('test_date', date('Y-m-d')) }}" required
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <select id="panelSelect" name="test_panel_id"
                ...
                <option value="{{ $id }}" data-category="{{ $panelCategories[$id] ?? '' }}">{{ $name }}</option>
                @endforeach
            </select>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('user.packages.create') }}" class="text-xs text-indigo-600 hover:text-indigo-700 font-medium">+ New</a>
            </div>
            <select id="packageSelect"
                class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none mb-3">
                <option value="">— Select package —</option>
                @foreach($myPackages as $pkg)
                <option value="{{ $pkg->id }}">⭐ {{ $pkg->name }} ({{ $pkg->tests_count }})</option>
                @endforeach
